...
public class RequisitionController {
    ...
    public ResponseEntity<Requisition> createRequisition(@RequestBody Requisition requisition) {
        ...
        requisition.setCreatedAt(java.time.LocalDateTime.now());
        requisition.setUpdatedAt(java.time.LocalDateTime.now());

        // Link items to this requisition for bidirectional relationship
        if (requisition.getItems() != null) {
            for (com.procurement.procurement.entity.procurement.RequisitionItem item : requisition.getItems()) {
